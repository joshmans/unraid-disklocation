<?php
	 ...
	 *
	 *  export_info_json.php - JSON equivalent of export_info_tsv.php, for scripting/integration
	 *  use cases (Home Assistant, Grafana, Homepage, etc.) where a TSV file is inconvenient.
	 */
	if(isset($_GET["download_json"])) {
		...
		foreach($raw_devices as $key => $data) {
			if(empty($data["status"]) && $data["groupid"]) {
				...
				$array = ( isset($_GET["raw_data_json"]) ? $data["raw"] : $data["formatted"] );
				...
				$type = ( isset($_GET["raw_data_json"]) ? "raw" : "formatted" );
				$listarray = list_array($array, $type, $physical_traynumber);
				
				// Build one JSON object per drive, keyed by the same stable column names used
				// elsewhere in the plugin (see get_table_order()), rather than positional columns.
				$drive_row = array("hash" => $hash);
				$arr_length = count($table_info_order_system);
				for($i=0;$i<$arr_length;$i++) {
					$drive_row[$table_info_order_system[$i]] = $listarray[$table_info_order_system[$i]];
				}
				$json_drives[] = $drive_row;
		...
		$json_output = array(
			"meta" => array(
				...
				"version" => DISKLOCATION_VERSION,
				"data_type" => ( isset($_GET["raw_data_json"]) ? "raw" : "formatted" ),
				"generated" => date("c"),
			),
			"drives" => $json_drives,
		);
		
		array_to_json_download($json_output, "disklocation-" . date("Ymd-His") . ( isset($_GET["raw_data_json"]) ? ".raw" : ".format" ) . ".json");
	}
?>
